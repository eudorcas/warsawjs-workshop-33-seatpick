<template>
    <div class="seat" @click="selectSeat" :class="{'active': isSelected, 'disabled': isReserved}"></div>
</template>
        ...
        methods: {
            selectSeat () {
                ...
                this.$emit('seat-selected', {
                    id: this.seatProp, 
                    type: this.isSelected ? 'add' : 'remove'
                });
            },
            checkReservations () {
                this.getReservedSeats.map(seat => {
                    if (seat === this.seatProp) {
